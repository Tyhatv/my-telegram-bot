...
async def is_member(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    try:
        member1 = await context.bot.get_chat_member(GROUP_ID, user_id)
        member2 = await context.bot.get_chat_member(SECOND_GROUP_ID, user_id)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_member(user_id, context):
        join_buttons = [[InlineKeyboardButton("Tham gia nhóm 1", url=GROUP_JOIN_LINK)],
                        [InlineKeyboardButton("Tham gia nhóm 2", url="https://t.me/kiemtienfree17")]]
        await update.message.reply_text("🚫 Bạn cần tham gia đủ cả 2 nhóm để sử dụng bot!",
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_member(user_id, context):
        join_buttons = [[InlineKeyboardButton("Tham gia nhóm 1", url=GROUP_JOIN_LINK)],
                        [InlineKeyboardButton("Tham gia nhóm 2", url="https://t.me/kiemtienfree17")]]
        await query.edit_message_text("🚫 Bạn cần tham gia đủ cả 2 nhóm để sử dụng bot!",
                                      reply_markup=InlineKeyboardMarkup(join_buttons))
        return
...
def main():
    ...
    # Tự động gửi top ref thứ 2 hằng tuần lúc 8h sáng
    aiocron.crontab("0 8 * * MON", func=lambda: send_weekly_top(app.bot))
